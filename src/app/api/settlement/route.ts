...
import { orders as orderStore, broadcastOrderUpdate, Order } from "../orders/sse/route"
import { updateOrderOnSui } from "@/lib/sui-storage"
...
async function settleOrder(order: Order): Promise<SettlementResult> {
    try {
        ...
        // Log settlement to Sui blockchain
        await updateOrderOnSui(order.id, `sui_${order.id}`, "settled", {
